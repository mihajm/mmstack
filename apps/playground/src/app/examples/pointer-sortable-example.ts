...
@Component({
  ...
  template: `
    <main>
      ...
      <p class="hint">Drag a row to reorder. Pointer-based (not native HTML5 drag).</p>
      ...
      <ul class="list" data-list="outer" [mmReorderable]="outer">
        @for (card of outer.items(); track card.id) {
          <li class="card" [attr.data-card]="card.id" [mmReorderableItem]="card">
      ...
      <ul class="list" data-list="t-outer" [mmReorderable]="tOuterCtl">
        @for (it of tOuterCtl.items(); track it.id) {
          <li class="item" data-titem [mmReorderableItem]="it">
            {{ it.label }}
            @if (it.id === 2) {
              <ul class="list tree-inner" data-list="t-inner" [mmReorderable]="tInnerCtl">
      ...
      <p class="hint">Drag toward the top/bottom edge — the container scrolls.</p>
  ...
  styles: `
    ...
    .grip {
      color: #9ca3af; cursor: grab; display: inline-flex;
      align-items: center; justify-content: center;
      padding: 8px; margin: -8px 0 -8px -4px; min-width: 24px; min-height: 24px;
      touch-action: none;
    }
    /* The one styling hook the engine exposes — fully overridable. */
    .item.mm-sortable-dragging {
      cursor: grabbing; box-shadow: 0 8px 24px rgb(0 0 0 / 18%);
      border-color: #c7d2fe; opacity: 0.95;
    }
    /* handle list: only the grip is grabbable, so the body must not look it */
    ul[data-list="pointer-handle"] .item { cursor: default; }
    ...
    .card-header {
      ...
      background: #f3f4f6; font-weight: 600; cursor: grab;
    }
    .card-header .grip { color: #9ca3af; margin: 0; padding: 0; min-width: 0; min-height: 0; }
    .nested {
      /* indent so the hierarchy reads as nested */
      margin-left: 14px; padding: 4px; min-height: 44px;
    ...
    .tree-inner {
      margin: 8px 0 0 16px; padding: 6px; min-height: 40px;
      background: #f9fafb; border-left: 2px solid #d1d5db; border-radius: 6px;
      padding-bottom: calc(6px + var(--mm-sortable-reserved, 0px));
    }
    .scroll-box { max-height: 180px; overflow: auto; border: 1px solid #e5e7eb; border-radius: 8px; padding: 6px; }
    .board { display: flex; gap: 16px; align-items: start; }
    .col {
      flex: 1; min-height: 80px; padding: 8px; background: #f9fafb; border-radius: 10px;
    ...
    .list-h { flex-direction: row; flex-wrap: nowrap; }
    .chip {
      padding: 10px 16px; background: #fff; border: 1px solid #e5e7eb;
      border-radius: 999px; cursor: grab; white-space: nowrap;
    }
    .chip.mm-sortable-dragging {
      cursor: grabbing; box-shadow: 0 8px 24px rgb(0 0 0 / 18%); border-color: #c7d2fe;
    }
  `,
})
export class PointerSortableExample {
  ...
  protected readonly scrollItems = signal<Task[]>(
    Array.from({ length: 15 }, (_, i) => ({ id: 100 + i, label: `Item ${i + 1}` })),
  );
