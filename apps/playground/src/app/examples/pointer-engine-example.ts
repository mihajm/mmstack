...
@Component({
  ...
  template: `
    <main>
      ...
      <p class="hint">
        ...
        <code>engine: 'pointer'</code> — driven by the pointer engine + unified session.
      </p>
      <div class="board">
  ...
  styles: `
    ...
    .zone {
      flex: 1; min-height: 140px; padding: 10px; border-radius: 10px;
      background: #f9fafb; border: 2px solid transparent;
      display: flex; flex-direction: column; gap: 8px;
    }
    .zone.over { border-color: #6366f1; background: #eef2ff; }
    .chip {
      padding: 10px 14px; background: #fff; border: 1px solid #e5e7eb;
      border-radius: 8px; cursor: grab;
    }
    .chip.dragging { box-shadow: 0 8px 24px rgb(0 0 0 / 18%); border-color: #c7d2fe; opacity: 0.95; }
  `,
})
export class PointerEngineExample {
  protected readonly buckets = signal<{ id: string; chips: Chip[] }[]>([
    { id: 'a', chips: [{ id: 1, label: 'One' }, { id: 2, label: 'Two' }] },
