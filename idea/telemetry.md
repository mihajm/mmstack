...
## Causality-substrate framing (2026-07-03 — positioning note for the docs wave)

The devtools seam + tracedSignal + §12 aim at one thing worth naming when documenting:
**"why" as a queryable trace** — interaction → write → derivation cascade → effect, with honest
timestamps in dev and aggregated graph-health metrics in prod. Component-tree devtools answer
*what re-rendered*; only a fine-grained graph whose node constructors we own can answer *why*.
That's the differentiator to lead with, and it's only possible because we own the primitives.

## Consent module (RFC §7): BUILT 2026-07-03

`consent.ts` in core: TrackingRequirement / ConsentStore (sync-or-async get, fire-and-forget set)
/ ConsentConfig on provideTelemetry; facade gains requirements/pending/consent signals + decide().
Shipped `localStorageConsentStore`. Semantics decided while building (all spec-pinned, 17 specs):
- emit-time gating, per (category, sink) requirement match; requirement.sink scopes to one sink.
- `required` (default): undecided/undeclared categories DROP (undeclared dev-warns once — the
  "can't track what you didn't request" lockstep); `implicit`: flow unless denied.
- uncategorized emits are never gated; spans gate per-sink but never defer (handle stays usable,
  ctx/correlation intact).
- async-store hydration: undecided emits DEFER (both modes — implicit must not race a stored
  denial), drain on settle/decide/hydrationTimeoutMs (default 5s); sync stores have no window.
- decide() persists via store, drains matching deferrals early; signal-backed requirements give
  delta re-consent (pending() = exactly the new items).
Core now at 90 specs. README consent section written.

## Second-pass adversarial review (2026-07-03, post-port): APPLIED

Three fresh-eyes reviewers (core facade, adapters+packaging vs installed SDK sources, spec
mutation-analysis) + fixes, all verified personally. Architecture held (every pillar survived
scrutiny; all findings implementation-tier). Fixed:
- **drop-timeout blackhole**: 'dropped' was terminal for the whole session; now buffer-only —
  live delivery resumes when the sink finally becomes ready.
- **injector-destroy cleanup**: DestroyRef cancels the drop timer, discards pending buffers,
  and calls the new optional `Sink.dispose()` (SSR/Lambda event-loop + route-provider leaks).
- **fault isolation everywhere**: throwing sinks/policies/factories are dev-warned and skipped,
  never break the caller; a throwing setError no longer masks the user's original exception.
- **honest buffered time**: facade stamps span start/end + log timestamps (SPI: startSpan gains
  startMs, SinkSpan.end gains endMs); flush replays in true FIFO across kinds.
- **otel**: 0.x peers widened (>=0.218 <1; ^0.218 already conflicted with npm latest 0.220),
  pagehide/visibilitychange flush + shutdown-on-dispose (metrics were 100% lost for tabs <60s),
  forced-id consume-once + try/finally, BYO map stores SpanContext (parent links survive parent
  end; FIFO-capped), metric name/kind conflict guard, error status message.
- **posthog/sentry**: nullable client types, handle keys namespaced by sink name (two instances
  no longer shadow each other).
- **spec gaps closed** (97 unit + 1 e2e now): active-span stack discipline (nested + throw),
  error()/metric() correlation, id shape/uniqueness, severity table, gauge/default kinds, falsy
  sink filtering, interceptor retry/sync-throw/duration-value, scope reactivity, vacuous asserts.
- core README quick start was broken (cold observable never subscribed) — rewritten.
Verified end-to-end again after fixes: unit suites, live collector smoke, real-browser playground
drive (tmp/telemetry-page-check.mjs), checkout trace queryable in Tempo.

**Redline decisions: RESOLVED + APPLIED 2026-07-03** (Miha approved attrs-first, delegated
auto-nest with a lean to yes; both implemented):
(1) emits are attrs-first: `event(name, attrs?, opt)` / `error(err, attrs?, opt)` /
`metric(name, value, attrs?, opt)` / `log(sev, msg, attrs?, opt)`; EmitOptions = {category,
parent}; SpanCallOptions keeps `attrs` (spans' attrs are genuinely optional).
(2) sync `span()` AUTO-NESTS under the active span (same stack that correlates emits);
`parent: null` forces a fresh root. `startSpan()` never auto-nests (manual spans + the HTTP
interceptor keep RFC §8 explicit/flat semantics). Both pinned by specs; READMEs updated.
This amends RFC §8's "explicit-only" for the synchronous span() form — update TELEMETRY-RFC.md
§10 when next touched.

## Port status (2026-07-03): DONE, pending release decisions

Ported to `packages/telemetry/{core,otel,posthog,sentry}` (form-style nested Nx projects,
`@mmstack/telemetry-core|-otel|-posthog|-sentry`, 22.0.0). Adapters peer-depend on core (DI token
identity must be singleton). 62 unit specs + 1 live-collector e2e, all green; build/lint green;
verbatim port held up under deep assertions except one real bug found+fixed: **otel BYO-provider
nesting split the exported trace** (child parented via facade ids OTel doesn't own) — now bridges
through the live OTel span, falls back flat+attrs after parent end. tracedSignal §8.2 sync-capture
semantics validated (last-writer-wins, no post-await leakage, zero-recomputation causality) and its
JSDoc hardened (@experimental kept). Close-the-loop harness: `scripts/telemetry-otlp-smoke.sh`
(grafana/otel-lgtm; KEEP=1 leaves Grafana up), OTLP_E2E-guarded e2e spec, playground `/telemetry`
page (memory sink in-page + OTLP out, interceptor + tracedSignal + TelemetryScope-ready wiring).

Open before tagging (Miha to redline):
- **DX: `event(name, opt)` vs RFC's `event(name, attrs)`** — recommend `event(name, attrs?, opt?)`
  (same for metric/log/error); mechanical change, specs pin behavior either way.
- Consent (RFC §7) is designed, not implemented — port-then-add decided; `category` is carried,
  non-gating (pinned by spec).
- READMEs written (worker register, wedge-first); docs pages deferred per billing switch.

## Current state in abscratch (2026-07-02, superseded by the port above)
