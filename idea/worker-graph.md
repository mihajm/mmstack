...
**Status: rungs 1–3 SHIPPED as `@mmstack/worker` (2026-07-03)** — workerResource (rung 1),
workerStore single-writer replicas over op-log deltas (rung 2), createWorkerHost derivations +
`published` mirrors with cross-boundary pending semantics (rung 3). Open-question outcomes:
worker runs its own graph (scheduling solved), DI stays on the main-thread edge, queryResource
`.arrayBuffer`/raw variants are the transferables seam. Remaining: rung 4 (app-builder execution
off-main — app-builder-side, mid-term) and the debuggability note, which now folds into
`idea/concurrency-devtools.md` (op traffic as a listener hook). Captured 2026-07-02.
...
## Beyond rung 4: server as a placement target (parked, 2026-07-03)

Discussion outcome on "server signals": ambient reactive state ON a server stays unconvincing
(Miha: servers are gather→return or subscription-shaped, and those models fit threads/streams
naturally). The defensible version needs no server-side reactive runtime at all: the server is
just another OWNER of store subtrees (mesh single-writer), recomputing on write over the op
stream and emitting leaf-granular invalidations. The signal-ness lives in the PROTOCOL's
granularity, not the server's execution model — internally it stays request/subscription-shaped.
Durable-Object-style actors are the natural host (a mesh room is the one culturally normal
long-lived server state). Park until the mesh relay exists; then "server-owned subtree" is a
small step, not a paradigm shift.
