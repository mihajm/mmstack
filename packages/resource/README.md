...
| ---------------------- | ------------------------------------------------------- | ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `defaultValue`         | `TResult`                                               | –                  | Initial value before the first request resolves. When set, `value()` is `TResult`, not `TResult \| undefined`. With `keepPrevious` it is only the value before anything was ever held.                                                                                        |
| `keepPrevious`         | `boolean`                                               | `false`            | Hold the previous `value`, `statusCode`, and `headers` across every gap — a reload, a new request identity (the next page), a fault. `hasValue()` follows the held value. Powered by `linkedSignal`.                                                                          |
...
- **`keepPrevious` holds across a new request identity too**, and `defaultValue` does not interfere: the default is the hold's fallback (what `value()` reads before the first result), not Angular's. Without the hold, Angular yields the default in every gap, the next page included — which is why a table with `defaultValue: []` and `keepPrevious: true` keeps page 1 on screen while page 2 loads. A cached identity still serves from cache immediately.
