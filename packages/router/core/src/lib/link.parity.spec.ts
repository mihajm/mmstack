...
// Tripwire: fails when Angular changes RouterLink's inputs/members so mmLink can be kept in sync.
// On failure, reconcile FORWARDED_INPUTS below with Link's hostDirectives in link.ts.
...
// Pins RouterLink's anchor detection via href reflection; on change, reconcile isAnchorLikeHost in link.ts.
