...
export type PanZoomRef = {
  /** The live space transform — satisfies the canvas `space` option. */
  transform: Signal<CanvasSpaceTransform>;
  panning: Signal<boolean>;
  /** viewport (client) point → canvas-space point. */
  toCanvas(clientPoint: { x: number; y: number }): { x: number; y: number };
  /** canvas-space point → viewport (client) point. */
  toViewport(canvasPoint: { x: number; y: number }): { x: number; y: number };
  reset(): void;
};

/**
 * Pan (drag, middle button by default) + zoom (wheel, around the cursor) for a
 * canvas viewport. Owns a `transform` signal — pass the ref as the canvas
 * `space` so gestures project through it live (zoom during a drag included);
 * apply it to your content layer via
 * `transform: translate(x, y) scale(scale)` with `transform-origin: 0 0`.
 */
export function panZoom(
  ...
  // host fallback only when no explicit viewport — usable from non-component injectors
  const host = viewport
    ? null
